...
void FastAppend(std::ofstream& dst, std::ifstream& src) {
	...
	while (!src.eof()) {
		size_t bytes_read = static_cast<size_t>(src.read(memory_buffer.data(), buffer_size).gcount());
...
int main(int argc, char **argv) {
	...
	std::for_each(fs::begin(dir_iterator), fs::end(dir_iterator), [&file_handles, &header](const fs::directory_entry&file) {
		if (file.status().type() == fs::file_type::regular) {
			...
			file_handles.push_back(
				std::make_tuple(std::ifstream(file_str, std::ios::binary), clean_file_str)
			);
			header.string_table_sz += static_cast<uint32_t>(clean_file_str.length() + 1);
			header.file_count++;
		}
	});
	...
	std::for_each(file_handles.begin(), file_handles.end(), [&nsp_fp, &file_offset, &string_table, &string_table_sz](std::tuple<std::ifstream, std::string> &entry) {
	...
	std::for_each(string_table.begin(), string_table.end(), [&nsp_fp](const std::string& str) {
		nsp_fp.write(str.c_str(), str.length());
	});
	...
	std::for_each(file_handles.begin(), file_handles.end(), [&nsp_fp, &indentation_level](std::tuple<std::ifstream, std::string> &entry) {
		auto &file = std::get<0>(entry);
		auto &filename = std::get<1>(entry);
		std::fill_n(std::ostreambuf_iterator<char>{std::cout}, indentation_level, ' ');
		std::cout << filename << " ...";
		// nsp_fp << file.rdbuf();
		FastAppend(nsp_fp, file); // Buffering instead of iterating per character is significantly faster
	...
	std::cout << "Took " << std::chrono::duration_cast<std::chrono::milliseconds>(difference).count() << "ms" << std::endl;
	return 0;
}
